...
function LeadsContent() {
  ...
  const suggestReply = async (id) => {
    setDrafting(true);
    try {
      const res = await api.post(`/leads/${id}/suggest_reply/`);
      setReplyText(res.data.draft || "");
    } catch {
      alert("AI drafting isn't available right now — write the reply manually instead.");
    }
    setDrafting(false);
  };
              ...
              {openReplyFor === lead.id && (
                <div style={{ marginTop: "12px", display: "flex", flexDirection: "column", gap: "8px" }}>
                  <div style={{ display: "flex", justifyContent: "flex-end" }}>
                    <button
                      onClick={() => suggestReply(lead.id)}
                      disabled={drafting}
                      style={{
                        padding: "5px 12px", background: "transparent", border: "1px solid var(--accent)",
                        color: "var(--accent)", borderRadius: "999px", fontSize: "12px", cursor: "pointer",
                      }}
                    >
